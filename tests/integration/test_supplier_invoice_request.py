"""Phase 2D.3-F1b — SUPPLIER_INVOICE_REQUEST rule foundation.

Covers the frozen supplier-direction rule layer (IP-P01..IP-P07, see
docs/PHASE2D3-RULE-FREEZE.md) over the F0 fact context, on
independently synthetic data:

- the IP-P02 expected amount == Contract.gross_amount, and an unknown
  amount produces an explicit missing-fact blocker (never an estimate);
- PURCHASE invoice cardinality: zero is a factual state with no false
  ...
  is the deterministic IP-P03 violation;
- the IP-P04 invoice -> contracts mapping and its violation blocker
  (never silently apportioned);
- exact amount and exact product-name consistency checks with
  MATCH / MISMATCH / NOT_COMPARABLE_MISSING_FACT — a MISMATCH is a
  factual outcome, never "unpaid"/"outstanding"/"overdue";
- OUT payments exposed as context only (IP-P01), no tax-rate inference
  (IP-P06), no quantity calculation (IP-P07);
...
def test_multiple_purchase_invoices_emit_ip_p03_conflict(db_session):
    frag = _make_fragment(db_session)
    ...
    assert set(decision.blockers[0].related_invoice_ids) == {invoice1.id, invoice2.id}
...
def test_invoice_on_multiple_contracts_emits_ip_p04_conflict(db_session):
    frag = _make_fragment(db_session)
    ...
    for contract in (contract_a, contract_b):
        ...
        ip_p04 = [b for b in decision.blockers if b.code == SupplierRequestBlockerCode.PURCHASE_INVOICE_SPANS_MULTIPLE_CONTRACTS]
        assert len(ip_p04) == 1
...
# ---------------------------------------------------------------------------
# D. IP-P02 amount consistency — MATCH / MISMATCH / NOT_COMPARABLE
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# F/G/H. IP-P01 payments as context, IP-P06 no tax inference, IP-P07 no quantity
# ---------------------------------------------------------------------------


def test_out_payment_context_does_not_change_status(db_session):
    ...
    and neither contract gains a payment-driven blocker."""
    frag = _make_fragment(db_session)
...
def test_no_tax_rate_inference_anywhere(db_session):
    ...
    decision = _decision_for(db_session, contract.id)
    # The Fact is displayed as it exists...
    assert decision.invoice_item_allocations[0].invoice_item.tax_rate == Decimal("0.13")
    # ...and no check or blocker carries any tax concept at all.
...
def test_status_and_dto_vocabulary_carry_no_business_judgment():
    # The decision status vocabulary has exactly the three factual /
    # preparation members — and none of the rejected judgment members.
    statuses = {
